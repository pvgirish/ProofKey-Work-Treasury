...
function buildEpochConfiguration(form: HTMLFormElement) {
  ...
  const config = {
    ...
    cap: integer("cap", true), policyHash: POLICY_HASH, initializationCutoff: integer("initializationCutoff", true), admissionCutoff: integer("admissionCutoff", true),
...
async function verifySettings(candidate: any) { for (const [side, config] of [["source", candidate.source], ["target", candidate.target]] as const) { signal(`${side}-config-signal`, "wait"); const provider = new ethers.JsonRpcProvider(config.rpcUrl); const [network, block] = await Promise.all([provider.getNetwork(), provider.getBlockNumber()]); if (network.chainId !== BigInt(config.chainId)) { signal(`${side}-config-signal`, "bad"); throw new Error(`${config.label} RPC reports chain ${network.chainId}, not ${config.chainId}`); } signal(`${side}-config-signal`, "good"); toast(`${config.label} reached at block ${block}`); } }
...
function createDraft(form: HTMLFormElement) {
  ...
  const bigint = (name: string) => { if (!/^(0|[1-9][0-9]*)$/.test(String(value[name]))) throw new Error(`${name} must be a nonnegative integer`); return BigInt(value[name]); };
  ...
  const milestone = { work: bigint("work"), fee: bigint("fee"), timeoutWork: bigint("timeoutWork"), deliverBefore: bigint("deliverBefore"), reviewBefore: bigint("reviewBefore"), ruleBefore: bigint("ruleBefore") };
  const acceptBefore = bigint("acceptBefore");
...
async function targetFunding(epochId: string, required: bigint) { const c = targetConfig(), source = sourceConfig(); const provider = new ethers.JsonRpcProvider(c.rpcUrl); const stable = await stableBlock(provider, Number(c.confirmations ?? 12)); const contract = new ethers.Contract(c.treasury, artifact("WorkTreasury").abi, provider); const [network, account, config] = await Promise.all([provider.getNetwork(), contract.epochAccount.staticCall(epochId, { blockTag: stable.blockTag }), contract.epochConfig.staticCall(epochId, { blockTag: stable.blockTag })]); if (network.chainId !== BigInt(c.chainId)) throw new Error("Target RPC chain changed"); if (!account.funded) throw new Error("Exact epoch is not funded on Creditcoin"); if (BigInt(account.reserve) < required) throw new Error("Target remaining reserve is below this milestone maximum"); const normalized = plain(config); const derived = ethers.keccak256(abi.encode(["bytes32", EPOCH_TUPLE], [EPOCH_TYPEHASH, normalized])); if (derived.toLowerCase() !== epochId.toLowerCase()) throw new Error("Target stored configuration does not derive this epoch ID"); if (BigInt(normalized.sourceChainId) !== BigInt(source.chainId) || BigInt(normalized.sourceChainKey) !== BigInt(source.chainKey) || normalized.sourceCoordinator.toLowerCase() !== source.coordinator.toLowerCase() || normalized.sourceSafe.toLowerCase() !== source.safe.toLowerCase() || BigInt(normalized.targetChainId) !== BigInt(c.chainId) || normalized.targetTreasury.toLowerCase() !== c.treasury.toLowerCase() || normalized.sourceVersion.toLowerCase() !== SOURCE_VERSION || normalized.schemaVersion.toLowerCase() !== SCHEMA_VERSION || normalized.policyHash.toLowerCase() !== POLICY_HASH) throw new Error("Target funded configuration does not match the pinned workspace domain"); return { block: stable.blockTag, blockLabel: stable.label, reserve: account.reserve, config: normalized, targetChainId: c.chainId }; }
...
async function signQuote() { if (!currentDraft) throw new Error("Create the draft first"); if (!connected) await connectWallet(); const required = BigInt(currentDraft.terms.milestones[0].work) + BigInt(currentDraft.terms.milestones[0].fee); const [funding, capacity] = await Promise.all([targetFunding(currentDraft.terms.epochId, required), sourceCapacity(currentDraft.terms.epochId, required)]); if (connected!.account.toLowerCase() !== currentDraft.terms.worker.toLowerCase()) throw new Error("The connected wallet is not the worker named in this quote"); const c = sourceConfig(); if (connected!.chainId !== BigInt(c.chainId)) throw new Error(`Switch the wallet to source chain ${c.chainId}`); const signature = await connected!.signer.signTypedData({ name: "ProofKey Source Coordinator", version: "1", chainId: BigInt(c.chainId), verifyingContract: c.coordinator }, { ProofKeyQuoteV1: [{ name: "orderId", type: "bytes32" }] }, { orderId: currentDraft.orderId }); currentDraft = { ...currentDraft, authority: "worker-signature", workerSignature: signature, targetFundingRead: { observedAt: new Date().toISOString(), block: funding.block, blockLabel: funding.blockLabel, reserve: String(funding.reserve), chainId: String(funding.targetChainId) }, sourceCapacityRead: { block: capacity.block, blockLabel: capacity.blockLabel, available: String(capacity.available), remainingAdmissions: capacity.remainingAdmissions } }; saveJson(DRAFT_KEY, currentDraft); showResult("draft-result", `Binding worker quote signed after independent target funding and source capacity reads.\nOrder ${currentDraft.orderId}\nTarget reserve ${funding.reserve} at ${funding.blockLabel}\nSource available ${capacity.available} · ${capacity.remainingAdmissions} admission slots at ${capacity.blockLabel}`, "success"); $<HTMLElement>("#prepare-quote").hidden = false; toast("Worker quote signed. The Safe must still accept it on Ethereum."); }
...
async function sendPrepared(tx: any, sourceSafe = false, requiredAccount?: string) { if (!connected) await connectWallet(); if (connected!.chainId !== BigInt(tx.chainId)) throw new Error(`Switch the wallet to chain ${tx.chainId}`); if (sourceSafe && connected!.account.toLowerCase() !== String(workspace.source.safe).toLowerCase()) throw new Error("The connected account is not the configured Safe. Open this app through the Safe wallet, or download the Safe transaction file."); if (requiredAccount && connected!.account.toLowerCase() !== requiredAccount.toLowerCase()) throw new Error(`This action requires ${short(requiredAccount)}; the connected wallet is ${short(connected!.account)}`); const sent = await connected!.signer.sendTransaction({ to: tx.to, value: BigInt(tx.value), data: tx.data }); toast(`Submitted ${short(sent.hash)}. Waiting for confirmation…`); await sent.wait(); toast(`Confirmed ${short(sent.hash)}`); return sent.hash; }
...
function bindEvents() {
  ...
  for (const [id, name] of [["action-deliver", "deliver"], ["action-approve", "approve"], ["action-challenge", "challenge"], ["action-no-delivery", "finalizeNoDelivery"], ["action-monitoring", "finalizeMonitoringDefault"], ["action-committee-timeout", "finalizeCommitteeTimeout"]]) $("#" + id).addEventListener("click", () => prepareGuidedSource(name).catch(error => toast(error.message, true)));
...
function boot() {
  ...
  if (currentDraft) { showResult("draft-result", `${currentDraft.authority === "worker-signature" ? "Binding worker quote" : "Nonbinding draft"} restored from this device.\nOrder ${currentDraft.orderId}`, "success"); $<HTMLButtonElement>("#sign-quote").disabled = currentDraft.authority === "worker-signature"; $<HTMLElement>("#prepare-quote").hidden = currentDraft.authority !== "worker-signature"; $<HTMLButtonElement>("#export-draft").disabled = false; }
