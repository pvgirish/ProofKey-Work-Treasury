...
- [Guided demo](https://pvgirish.github.io/ProofKey-Work-Treasury/demo.html) — historical testnet walkthrough that needs no wallet.
- [Video player](https://pvgirish.github.io/ProofKey-Work-Treasury/video.html) — edited walkthrough with synthetic narration and captions.
- [Submission PDF](docs/media/ProofKey-Work-Treasury-Final-2026-09-14.pdf) — the ten-page practice-token submission deck.
- Created by the sole human creator, Venkatagirish Pamidigantam, with AI assistance from Codex and Claude.
...
The baseline financial implementation and team-controlled public demonstrations are complete: all four testnet epochs closed with 143 CTC deposited, 143 CTC withdrawn and zero remaining liabilities. The September 10 participant-product release is the public baseline. Current public CI runs its 47 contract, 59 SDK, 6 participant and 21 evidence-checker tests; see [release run 34785473521](https://github.com/pvgirish/ProofKey-Work-Treasury/actions/runs/34785473521), the [finalized cross-chain audit](evidence/release-readback.json), and [release gates](docs/RELEASE-GATES.md). These counts refer to the preserved published baseline, not broader unpublished Q2.1 test totals. The [participant-workflow upgrade](docs/PRODUCT-UPGRADE-PROGRESS.md) adds portable consent, replacement settlement and program closeout to the local build, with the financial contracts unchanged. Its local validation is separate from the historical hosted release. Two consenting independent settlements, repeat independent use and a real buyer reason remain pending.
